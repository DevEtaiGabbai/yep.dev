...
export const FileTree = ({ 
  ...
  const [fileTree, setFileTree] = useState<FileNode>({ name: '/', path: '', type: 'directory', children: [] });
  ...
  useEffect(() => {
    ...
    const getOrCreateDir = (pathParts: string[], currentNode: FileNode): FileNode => {
      if (pathParts.length === 0) return currentNode;
      
      const firstPart = pathParts[0];
      const remainingParts = pathParts.slice(1);
      const path = currentNode.path ? `${currentNode.path}/${firstPart}` : firstPart;
      
      // Try to find existing child node
      let childNode = currentNode.children.find(node => node.name === firstPart);
    ...
    Object.entries(files).forEach(([path, file]) => {
      ...
      if (!path || path === '/' || path.startsWith('/.bolt') || path === '/.bolt' || path.startsWith('/dist') || path === '/dist') return;
      ...
      if (pathParts[0] === '.bolt' || pathParts[0] === 'dist' || pathParts[0] === 'node_modules') return;
      
      // For files, we add to parent directory 
      if (file.type === 'file') {
        const fileName = pathParts.pop()!;
        const parentNode = pathParts.length ? getOrCreateDir(pathParts, root) : root;
