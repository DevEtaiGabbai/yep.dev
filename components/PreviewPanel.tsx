...
export const PreviewPanel = ({ previews, isLoading, initialRoute = '/' }: PreviewPanelProps) => {
  const [selectedPort, setSelectedPort] = useState<number | null>(previews.length > 0 ? previews[0].port : null);
  const [route, setRoute] = useState<string>(initialRoute);
  const [isPortDropdownOpen, setIsPortDropdownOpen] = useState(false);
  const [iframeKey, setIframeKey] = useState<number>(0);

  const selectedPreview = previews.find(p => p.port === selectedPort) || previews[0];
  const iframeSrc = selectedPreview ? selectedPreview.url.replace(/\/$/, '') + route : '';
    ...
    return (
      <div className="h-full flex items-center justify-center bg-[#101012]">
        <div className="text-center max-w-md px-4">
          ...
          <h3 className="text-lg font-medium text-[#f3f6f6] mb-3">Starting preview server...</h3>
          <p className="text-sm text-[#969798]">
            This process should take a few moments while we&apos;re setting up the web container and starting the dev server.
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div className="h-full flex items-center justify-center bg-[#101012]">
        <div className="text-center max-w-md px-4">
          ...
          <h3 className="text-lg font-medium text-[#f3f6f6] mb-2">Preview server failed to start</h3>
          <p className="text-sm text-[#969798]">
            The development server couldn&apos;t be started. Please check the terminal for error messages or try refreshing the page.
          </p>
        </div>
      </div>
    );
          ...
          <div className="flex-1 flex items-center gap-1 bg-[#212122] border border-[#313133] rounded-lg px-3 py-1.5 focus-within:ring-1 focus-within:ring-[#464649] group">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger className="flex items-center gap-1 text-[#969798] hover:text-[#f3f6f6] pr-2 border-r border-[#313133] focus:outline-none">
                <span className="text-xs">{selectedPort || (previews.length > 0 ? previews[0].port : 'Port')}</span>
                ...
                {previews.map(p => (
                  <DropdownMenuItem 
                    ...
                    className={cn(
                      ...
                      p.port === selectedPort ? "text-[#f3f6f6] bg-[#2a2a2c]" : "text-[#969798]"
                    )}
            ...
            <input
              ...
              value={route}
              onChange={e => setRoute(e.target.value.startsWith('/') ? e.target.value : '/' + e.target.value)}
              placeholder="/"
            />
          </div>
