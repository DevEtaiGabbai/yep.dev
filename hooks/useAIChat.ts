...
import { OPENROUTER_API_URL, DEFAULT_MODEL, MAX_TERMINAL_EXECUTION_TIME } from '@/lib/constants';
...
export const useAIChat = (
  ...
  const parseContentForFiles = async (content: string) => {
    ...
    if (state.insideAction && state.actionType === 'file' && state.actionFilePath) {
      const closeTagIndex = content.indexOf(BOLT_ACTION_TAG_CLOSE, state.currentActionStartIndex);
      
      if (closeTagIndex !== -1) {
        // We found the closing tag, extract the complete content
        const fileContent = content.substring(state.currentActionStartIndex, closeTagIndex);
        ...
        if (partialContent.length > 0 && partialContent !== state.partialContent) {
      ...
      if (actionType === 'file') {
        ...
        if (filePath) {
          ...
          if (state.completedFiles.has(filePath)) {
            ...
            const skipCloseIndex = content.indexOf(BOLT_ACTION_TAG_CLOSE, tagEndIndex);
            if (skipCloseIndex !== -1) {
          ...
          const closeTagIndex = content.indexOf(BOLT_ACTION_TAG_CLOSE, tagEndIndex);
          
          if (closeTagIndex !== -1) {
            // Complete file found
            const fileContent = content.substring(tagEndIndex + 1, closeTagIndex);
    ...
    try {
      ...
      if (data.type === 'progress') {
        ...
        setStreamingData((prev: any) => {
          ...
          const existingIndex = progressUpdates.findIndex((p: any) => p.order === data.order);
  ...
  const writeFileToWebContainer = async (webContainer: WebContainer, file: GeneratedFile) => {
    try {
      // Create parent directory if needed
  ...
  const processShellCommands = async (content: string) => {
    ...
    const shellCmdRegex = /<boltAction\s+type="shell">([\s\S]*?)<\/boltAction>/g;
    let shellMatch;
    ...
    const filteredCommands = commands.filter(cmd => {
      const isDevServer = cmd.trim() === 'npm run dev' || 
                         cmd.trim() === 'npm start' ||
    ...
    try {
      ...
      for (const command of filteredCommands) {
        ...
        await runTerminalCommand(`echo \r\n\u001b[1;34m▶ Running command: ${command}\u001b[0m\r\n\u001b[1;32mdev server will restart automatically\u001b[0m`, 'bolt-setup');
        ...
        try {
          ...
          const timeoutPromise = new Promise((_, reject) => {
            setTimeout(() => {
              reject(new Error(`Command timed out after ${MAX_TERMINAL_EXECUTION_TIME/1000}s: ${command}`));
          ...
          await runTerminalCommand(`echo "\r\n\u001b[1;31m✖ Command timed out: ${command}\u001b[0m"`, 'bolt-setup');
        }
  ...
  const processSSEStream = async (reader: ReadableStreamDefaultReader<Uint8Array>, decoder: TextDecoder) => {
  ...
  const finalizeFileProcessing = async (finalContent: string) => {
    ...
    try {
      ...
      const fileRegex = new RegExp(`${BOLT_ACTION_TAG_OPEN}\\s+type="file"\\s+filePath="([^"]+)">((?:[\\s\\S](?!${BOLT_ACTION_TAG_CLOSE}))*?[\\s\\S]?)${BOLT_ACTION_TAG_CLOSE}`, 'g');
      let match;
