...
const getPersistentShellDimensions = (shell: any) => {
  try {
    if (!shell || !shell.isInitialized()) {
      console.warn("Shell not initialized when getting dimensions, using default.");
      return { cols: 80, rows: 24 };
    }
    return shell.getDimensions();
  } catch (e) {
    console.warn("Could not get persistent shell dimensions, using default.", e);
    return { cols: 80, rows: 24 };
  }
};
...
export const useWebContainer = (terminalRef?: MutableRefObject<TerminalRef | null>) => {
  ...
  const runTerminalCommand = useCallback(async (command: string, terminalId: string) => {
    if (!webContainerInstance || !persistentShell.isInitialized()) {
      const errorMsg = !webContainerInstance ? 'Web container not available' : 'Shell not initialized';
  ...
  const runNpmInstall = useCallback(async () => {
    ...
    if (!webContainerInstance || isInstallingDeps) { // Removed shell check here, spawn directly
  ...
  }, [webContainerInstance, isInstallingDeps, terminalRef, pipeProcessOutputToTerminal]);

  // Start the dev server in the 'main' (Bolt) terminal
  const startDevServer = useCallback(async () => {
    const terminalId = 'main';
    if (!webContainerInstance || isStartingDevServer || devServerProcess) { // Removed shell check
      console.warn("Skipping dev server start: WC not ready, already starting/running.");
      return;
    }
    ...
    try {
      ...
      const serverReadyListener = (port: number, url: string) => {
        if (!serverReadyCalled) {
          ...
          setPreviews(prev => {
            ...
            if (exists) return prev.map(p => (p.port === port ? { port, url } : p));
      ...
      const disposeServerReady = webContainerInstance.on('server-ready', serverReadyListener);

      process.exit.then(exitCode => {
        console.log(`Dev server process exited with code ${exitCode}`);
        disposeServerReady();
        setDevServerProcess(null);
        if (!serverReadyCalled) { // Only update state if server-ready wasn't called
          setIsStartingDevServer(false);
          terminalActions.setTerminalRunning(terminalId, false); // Update store state
          terminalActions.setTerminalInteractive(terminalId, true);
        }
  ...
  }, [webContainerInstance, isStartingDevServer, devServerProcess, terminalRef, pipeProcessOutputToTerminal]);
  ...
  const stopDevServer = useCallback(async () => {
    ...
    if (devServerProcess) {
      ...
      terminalActions.setTerminalRunning(terminalId, true, 'Stopping server...');
