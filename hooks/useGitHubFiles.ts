...
export const useGitHubFiles = (webContainerInstance: WebContainer | null, customRepoName?: string) => {
  ...
  const [filesystemMonitorId, setFilesystemMonitorId] = useState<number | null>(null);
  ...
  useEffect(() => {
    const initializeFiles = async () => {
      ...
      try {
        ...
        const defaultFile = fetchedFiles.find(f => 
        ...
        setGitHubError(`Failed to load files from ${repoName}: ${error.message}`);
        
        // Check for rate limit in error message
        if (error.message.includes('rate limit')) {
          const rateLimitMatch = error.message.match(/GitHub API rate limit exceeded. Resets at (.*)/);
  ...
  const scanFileSystem = useCallback(async (webContainer: WebContainer) => {
  ...
  }, [webContainerInstance, isLoadingGitHubFiles, scanFileSystem, selectedFile]);
