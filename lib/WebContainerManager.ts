...
class WebContainerManager {
   ...
   */
  public async initializeShell(terminalRef?: TerminalRef | null): Promise<void> {
    try {
      if (!this.webContainer) {
        console.log("initializeShell: WebContainer not ready, booting first...");
        await this.getWebContainer(); // Ensure WebContainer is booted
      }

      if (!this.webContainer) {
        throw new Error('WebContainer not available after boot attempt for shell init');
      }
      ...
      if (terminalRef) {
        ...
        console.log('WebContainerManager: Terminal ref validation:', {
          ...
          dimensions,
          isValidTerminal: hasMethods && dimensions.cols > 0 && dimensions.rows > 0
        });

        // If dimensions are invalid, attempt to force a resize
        if (dimensions.cols === 0 || dimensions.rows === 0) {
          console.log('WebContainerManager: Terminal has invalid dimensions, attempting to resize...');
          try {
            terminalRef.resize();
            // Wait a moment and check again
            await new Promise(resolve => setTimeout(resolve, 100));
            dimensions = terminalRef.getDimensions();
            console.log('WebContainerManager: After resize, dimensions:', dimensions);
          } catch (e) {
          ...
          if (dimensions.cols <= 0 || dimensions.rows <= 0) {
            console.log('WebContainerManager: Using default dimensions 80x24 after resize failed');
      ...
      if (!this.persistentShell.isInitialized()) {
        if (!terminalRef) {
          console.log("WebContainerManager: No terminal ref available, deferring shell initialization...");
        ...
        while (attempts < maxAttempts) {
          try {
            ...
            try {
              ...
              console.log('WebContainerManager: Using dimensions for shell init:', dimensions);
            } catch (e) {
              console.warn('Failed to get dimensions for shell init, using defaults:', e);
            }
        ...
        console.log("WebContainerManager: Restoring shell with new terminal ref...");
